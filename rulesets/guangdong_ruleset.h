...
#include <guangdong_bonus_tiletype_counter.h>
...
class GuangdongRuleset final : public Ruleset<34, 8, GuangdongCommonTiletypeCounter, GuangdongBonusTiletypeCounter> {
 public:
  const static uint CommonTN = 34;
  const static uint BonusTN = 8;
  typedef GuangdongCommonTiletypeCounter CommonTC;
  typedef GuangdongBonusTiletypeCounter BonusTC;

  GuangdongRuleset() : Ruleset() {
    ...
    std::vector<RawRule<CommonTN, BonusTN, CommonTC, BonusTC>*> raw_rule_ptr_vct =
        std::vector<RawRule<CommonTN, BonusTN, CommonTC, BonusTC>*>{
            /* 一番 */
            // TODO: 自摸
            new RawIncrementalRule<CommonTN, BonusTN, CommonTC, BonusTC>(
                "三元牌（一刻）", 1,
                std::vector<std::vector<Tiletype>>{
                    ...
                    //                Tiletype::dragonGreen}, std::vector<Tiletype>{Tiletype::dragonRed,
                    //                Tiletype::dragonRed, Tiletype::dragonRed},
                },
                std::vector<std::tuple<uint, uint, uint>>{
            ...
            new RawIncrementalRule<CommonTN, BonusTN, CommonTC, BonusTC>(
                "明杠", 1, std::vector<std::vector<Tiletype>>{std::vector<Tiletype>()},
                std::vector<std::tuple<uint, uint, uint>>{
                    std::make_tuple(1, 0, 4),
                    std::make_tuple(1, 1, 3),
                    std::make_tuple(1, 2, 2),
                ...
                [](const State<CommonTN, BonusTN, CommonTC, BonusTC>& state) { return state.exposed_kong_num_ >= 1; }),
            new RawFunctionOnlyRule<CommonTN, BonusTN, CommonTC,
                                    BonusTC>("無花", 1,
                                             [](const State<CommonTN, BonusTN, CommonTC, BonusTC>& state) {
                                               for (uint idx = 0; idx < state.bonus_tiletype_counter_.counter_size_ * 3;
                                                    idx += 1) {
                                                 if (state.bonus_tiletype_counter_.counter_[idx]) {
                                                   return false;
                                                 }
                                               }
                                               return true;
            ...
            // TODO: 正花
            new RawFunctionOnlyRule<CommonTN, BonusTN, CommonTC, BonusTC>(
                "門清", 1,
                [](const State<CommonTN, BonusTN, CommonTC, BonusTC>&
                       state) { return state.exposed_sequence_num_ == 0 and state.exposed_triplet_num_ == 0; }),
            // TODO: 海底撈月
            // TODO: 海底捞鱼
            // TODO: 絕張
            // TODO: 生張
            ...
            new RawIncrementalRule<CommonTN, BonusTN, CommonTC, BonusTC>(
                ...
                std::vector<std::vector<Tiletype>>{
                    ...
                    std::vector<Tiletype>{Tiletype::pinSeven, Tiletype::pinEight, Tiletype::pinNine, Tiletype::pinSeven,
                    ...
                    std::vector<Tiletype>{Tiletype::manSeven, Tiletype::manEight, Tiletype::manNine, Tiletype::manSeven,
            ...
            new RawIncrementalRule<CommonTN, BonusTN, CommonTC, BonusTC>(
                ...
                [](const State<CommonTN, BonusTN, CommonTC, BonusTC>& state) {
                  return state.exposed_kong_num_ + state.concealed_kong_num_ >= 2;
                }),
            new RawIncrementalRule<CommonTN, BonusTN, CommonTC, BonusTC>(
                "清四碰", 1, std::vector<std::vector<Tiletype>>{std::vector<Tiletype>()},
                std::vector<std::tuple<uint, uint, uint>>{
                    std::make_tuple(1, 0, 4),
                },
                [](const State<CommonTN, BonusTN, CommonTC, BonusTC>& state) {
                  return state.exposed_triplet_num_ == 4;
                }),
            // TODO: 四歸一
            // TODO: 半求人
            new RawIncrementalRule<CommonTN, BonusTN, CommonTC, BonusTC>(
                ...
                std::vector<std::vector<Tiletype>>{
                    ...
                    std::vector<Tiletype>{Tiletype::pinSix, Tiletype::pinSeven, Tiletype::pinEight, Tiletype::manSix,
                                          Tiletype::manSeven, Tiletype::manEight},
                    std::vector<Tiletype>{Tiletype::pinSeven, Tiletype::pinEight, Tiletype::pinNine, Tiletype::manSeven,
                                          Tiletype::manEight, Tiletype::manNine},
                },
                std::vector<std::tuple<uint, uint, uint>>{
                    std::make_tuple(1, 0, 2),
            ...
            /* 三番 */
            // TODO: 門風, 圈風, 花槓, 暗槓, 全求人, 門清自摸, 槓上開花, 雙暗刻, 斷幺九, 一條龍, 三色同順,
            //  五門齊, 四歸二, 混帶夭九, 雙同刻, 三尾, 三料牌, 斷紅
            new RawIncrementalRule<CommonTN, BonusTN, CommonTC, BonusTC>(
                "平和", 3,
                std::vector<std::vector<Tiletype>>{
            ...
            // TODO:
            /* 四十番 */
            // TODO: 特殊：组合龍, 三元七對, 七搶一, 全不靠, 荒牌十番, 四喜七對, 八仙過海, 十三魔殿, 十三金字塔,
            //  十三騎士, 十三幺, 連七對,
  ...
  uint getTileNum() { return 14; };
  const std::string& getCachedTiletypeCountersByRulesPath() {
    return cached_tiletype_counters_by_rules_path_singleton_;
  };

  const inline static Tileset tileset_singleton_ = GuangdongTileset();
  const inline static std::string& cached_tiletype_counters_by_rules_path_singleton_ =
      "guangdong_tiletype_counter_set_by_rules";
};
